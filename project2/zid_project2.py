...
def read_prc_csv(tic):
    ...
    """
    #ensure that the tic is fully lowercase to match file names
    tic_lower = tic.lower()
    # use DATADIR imported from config file to create a new datafiled for the tic entered
    df = pd.read_csv(os.path.join(cfg.DATADIR, '{}_prc.csv'.format(tic_lower)))

    #set the date col as the index
    df = df.set_index("Date")
    df.index = pd.to_datetime(df.index)

    #standardise the names by importing the standardise_colnames function from config
...
def mk_prc_df(tickers, prc_col='adj_close'):
    ...
    **not** necessarily represent the actual contents of the data frame you
    ...
    #merge dfs for tics in tickers
    for tic in tickers:
        df_full = read_prc_csv(tic)
        df_prc_col = df_full[prc_col]
        df_output = pd.merge(df_prc_col, df_output, how='outer',left_index=True, right_index=True)

    # need to change the col names to a lower case version of ticker list
    lower_list = [tic.lower() for tic in tickers] # make new all lowercase list
    name_map = {old: new for old, new in zip(df_output.columns, lower_list)} #create dict of new to old names
    df_output_correct_col_names = df_output.rename(columns=name_map) # update cols

    return df_output_correct_col_names
...
def mk_ret_df(prc_df):
    ...
    """
    #calc the differences for the stocks
    pct_diff = prc_df.pct_change()

    # get market data and ensure it's format is aligned with stocks
    market_df_full = pd.read_csv(cfg.FF_CSV)
    market_df_full_date_indexed = market_df_full.set_index("Date")
    market_df_full_date_indexed.index = pd.to_datetime(market_df_full_date_indexed.index)
    market_df_col = market_df_full_date_indexed['mkt']

    # use inner merge because "include all dates in `prc_df` which are **also** present in the CSV file FF_CSV"
    df_output = pd.merge(pct_diff, market_df_col, how='inner', left_index=True, right_index=True)
    return df_output
...
def mk_aret_df(ret_df):
    ...
    """
    # because we don't know how many tickers will be in the analysis, but we do know that the mkt returns will be
    # in the last column, we create variable ticker containing all tickers
    tickers = ret_df.columns[:-1]
    abnormal_returns_datafield = ret_df[tickers].sub(ret_df.iloc[:, -1], axis=0)
    return abnormal_returns_datafield
...
def get_avg(df, col, year):
    ...
    """
    av_year = df.loc[df.index.year == year, col].mean()
    return av_year
...
def get_ew_rets(df, tickers):
    ...
    """
    av_portfolio_returns_df = df[tickers].mean(axis=1)
    return av_portfolio_returns_df


def get_ann_ret(ser, start, end):
    """ Returns the annualised returns for a given period.

    Giv`en a series with daily returns, this function will return the
    ...
    """
    ser_add_1 = ser + 1 #need to do this as returns are shown as t1/t0 - 1
    date_bound_subset_df = ser_add_1.loc[start:end] # create new dataset starting and ending with start/end
    total_returns = date_bound_subset_df.prod() # calculate product of new dataset
    not_null_entries = date_bound_subset_df.notnull().sum() # count number of notnull entries
    annualized_returns = (total_returns)**(252/not_null_entries) -1 #calc returns
...
## Jackson Test
# tickers = tickers = ['AAPL', 'TSLA']
# prc_df = mk_prc_df(tickers, prc_col='adj_close')
# mk_ret_datafield = mk_ret_df(prc_df)
# y = mk_aret_df(mk_ret_datafield)
# print(y)
